...
function [system_alt t] = Zero_model(payload_mass, volume, mass_balloon, mass_helium, plot_suppression)
% inputs :
...
R = 287.058;                              % Gas constant of ideal gases [m^3 Pa]/[K mol]
...
Mtot = payload_mass + mass_balloon + mass_helium(end);   %Total Mass of the whole system

num_moles = mass_helium(end) / mol_mass;   %mol ratio (constant, super pressure balloon)

num_cylinders = mass_helium(end) / cylinder_coef;
...
diameter = 0;
 ...
 while (system_alt(end) < 100000 && t(end) < 20000 )%&& velocity > 0)   
    ...
    if i ~= 1
        rho_g = P_air / (R * Tg(end));
        mass_helium = [mass_helium, rho_g * volume];
    end
    
    %Find the Boyant force
    Fb = (mass_helium(end) * mol_ratio * temp_ratio - Mtot) * g; %reference eq 2.94 from Engineering Fundamentals of balloons combines bouyant and gravity
