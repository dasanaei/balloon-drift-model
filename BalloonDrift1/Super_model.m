...
%6.5 m diameter burst diameter
%Latex
%6 lbs payload (2.72155 kg)
%750 g balloon 
%Go till burst
%mass_helium = 2.0018
...
function [Performance] = Super_model( payload_mass, burst_dia, mass_balloon, mass_helium, plot_suppression)
% inputs :
%   payload_mass:       wanted payload mass in kg
%   burst_dia:          Burst diameter in m
...
free_lift_ratio = [];                   % ratio of balloon lift to system mass
...
Vol_max = (4/3) * pi * burst_dia^3;       %Largest volume of the Balloon
...
diameter = 0;
 ...
 while (system_alt(end) < 100000 && diameter(end) < burst_dia && t(end) < 40000)   
    ...
    %Determining the Volume of the Balloon and other physical dimensions
    [temp_balloon_vol, diameter_temp] = findVol3(mass_helium, Tg, Vol_max, system_alt(end));    %Determins the volume and the diameter of the Balloon
    balloon_vol = [balloon_vol, temp_balloon_vol];
    ...
    %find velocity
    velocity = sqrt(velocity^2 + 2 * acc_temp * delta_t);
...
if (plot_suppression ~= 0) 
    ...
    figure(2)
    plot(system_alt(2:end), balloon_vol);
    xlabel('Altitude (m)');
    ylabel('Balloon Volume (m^3)');
    title('Balloon Volume vs Altitude for given Payload Mass')
end
