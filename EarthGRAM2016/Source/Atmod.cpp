//P. White
//Earth-GRAM 2016, Atm Class
//The atmod class computes mean and standard deviation pressure, density, 
//temperature and winds for output.  
//As well as perturbed values and atmospheric constituents.
...
void Atm::initializeMemberVariables()
{
...
void Atm::pert()
{
	...
	//for pressure (ppert), density (dpert), temperature(tpert), east-west wind (upert), 
	//north-south wind (vpert), and vertical wind (wpert).
	msisa.maps.perts.pert1(h, phi, thet, iupdate, nm);
...
void Atm::met()
{
	...
	double pmm, dmm, tmm, umm, vmm, wmm, arnd1, hend1, hnd1, o2nd1, n2nd1,
		ond1, wtmol1;
	...
	msisa.maps.perts.iperts.mets.jacmod(h, phi, thet, elt, &pmm, &dmm, &tmm,
		&umm, &vmm, &wmm, &arnd1, &hend1, &hnd1, &o2nd1, &n2nd1, &ond1,
		&wtmol1);
...
void Atm::jb2008()
{
	...
	double pmm, dmm, tmm, umm, vmm, wmm, n2nd1, o2nd1, ond1, arnd1, hend1, hnd1, wtmol1, dmdz2, nnd1;
...
void Atm::msis()
{
	...
	double pmm, dmm, tmm, umm, vmm, wmm, n2nd1, o2nd1, ond1, arnd1, hend1, hnd1,
		wtmol1, dmdz2, nnd1;
	double pi = 3.1415926535897931, g0, g, re, r0, ri1;
	double pi180 = pi / 180;
	double phir = phi * pi180;
	...
	//density, temperature, east-west wind, north-south wind, and vertical wind.  
	//Atmospheric constituents ar, he, h, o2, n2, and o are also calculated.
	msisa.msismod(h, phi, thet, elt, ri1, g, &pmm, &dmm, &tmm, &umm, &vmm, &wmm,
		&n2nd1, &o2nd1, &ond1, &arnd1, &hend1, &hnd1, &wtmol1, &dmdz2, &nnd1);
...
double Atm::tdbuck(double t, double rh, double p)
{
...
double Atm::wexler(double t, double p)
{
...
double Atm::mixrat(double tx, double p)
{
...
void Atm::fair(int i, double hg, double pg, double dg, double tg, double ug, double vg, double wg,
...
void Atm::stdatm(double z, double *t, double *p, double *d)
{
...
void Atm::atmos()
{
	...
	//atmos member function for the atmod class
	...
	double czi, pmm, dmm, tmm, umm, vmm, wmm, sec, water, dest,
	...
	int f1 = 0, i, imin, ihr = msisa.maps.perts.iperts.inits1.ihro;
	...
	msisa.maps.perts.iperts.inits1.out << "  Height/  GcLat    Long.                        Tempera-               Vert." << '\n' <<
	...
	for (int j = 0; j < msisa.maps.perts.iperts.inits1.mc; j++){
		...
		i = 0;
		nm = 0;
		//If iopt > 0, trajectory input data is used.
		if (msisa.maps.perts.iperts.inits1.iopt > 0){
			//Open and read traj file.
			msisa.maps.perts.iperts.inits1.trajopen();
			msisa.maps.perts.iperts.inits1.traj();
			//Set time and position based off of traj file.
			h = msisa.maps.perts.iperts.inits1.hgt;
			phi = msisa.maps.perts.iperts.inits1.lat1;
			thet = msisa.maps.perts.iperts.inits1.lon1;
			elt = msisa.maps.perts.iperts.inits1.time1;
			//Check for traj eof.
			f1 = msisa.maps.perts.iperts.inits1.f;
			msisa.maps.perts.iperts.inits1.nmax = 0;
		}
		//If traj is not used, use namelist file inputs.
		else {
			h = msisa.maps.perts.iperts.inits1.h1;
			phi = msisa.maps.perts.iperts.inits1.phi1;
			thet = msisa.maps.perts.iperts.inits1.thet1;
		}
		...
		if (h <= hg2){
			...
			if (h < hg1){
				...
				//Call intruv member function from InitPert class to calculate 
				//standard deviation of winds from MAP data.  
				//Used to calculate spdavg and spdsd.
				...
				//Calculate a faired value for wind speed and wind speed standard 
				//deviation.
		...
		msisa.maps.perts.iperts.inits1.out << fixed << setw(9) << setprecision(3) << h
			...
			setw(6) << setprecision(3) << msisa.maps.perts.iperts.uvth << setw(21) <<
			...
			<< setw(5) << setprecision(1) << spdgh << setw(21) << setprecision(2) << sphp << "%" << setw(11) <<
		...
		//Continue to build trajectory after initial state is calculated.
		//Loop through additional steps based on user-selected number of steps
		//with 'nmax' or trajectory eof variable f1.
		while ((f1 != 0) || (i < msisa.maps.perts.iperts.inits1.nmax - 1)){

			//If iopt > 0, trajectory input data is used.
			if (msisa.maps.perts.iperts.inits1.iopt > 0){
				//Read traj data and set position and time variable.
				msisa.maps.perts.iperts.inits1.traj();
				f1 = msisa.maps.perts.iperts.inits1.f;
				phi = msisa.maps.perts.iperts.inits1.lat1;
				thet = msisa.maps.perts.iperts.inits1.lon1;
				elt = msisa.maps.perts.iperts.inits1.time1;
				msisa.maps.perts.iperts.inits1.dphi = msisa.maps.perts.iperts.inits1.dphi1;
				msisa.maps.perts.iperts.inits1.dthet = msisa.maps.perts.iperts.inits1.dthet1;
				msisa.maps.perts.iperts.inits1.delt = msisa.maps.perts.iperts.inits1.delt;
				msisa.maps.perts.iperts.inits1.dhgt = msisa.maps.perts.iperts.inits1.dz1;
				h = msisa.maps.perts.iperts.inits1.hgt1;
				if (f1 == 0) return;
			}
			else {
				//If traj data is not used set time and position data based on
				//namelist file.
				dz = msisa.maps.perts.iperts.inits1.dhgt;
				dthet = msisa.maps.perts.iperts.inits1.dthet;
				dphi0 = dphi;
				h = h + dz;
				phi = phi + dphi0;
				thet = thet + dthet;
				elt = elt + msisa.maps.perts.iperts.inits1.delt;
				imin = msisa.maps.perts.iperts.inits1.mino + int(elt) / 60;
				sec = msisa.maps.perts.iperts.inits1.seco + elt;
				sec = int(sec) % 60;
				ihr = msisa.maps.perts.iperts.inits1.ihro + imin / 60;
				imin = imin % 60;
			}
			...
			msisa.maps.perts.iperts.inits1.out << fixed << setw(9) << setprecision(3) << h
				...
				setw(6) << setprecision(3) << msisa.maps.perts.uvt2 << setw(21) <<
				setprecision(2) << prhp << "%" << setw(11) << drhp << "%" << setw(8) <<
	...
	output.close();
	species.close();
	msisa.maps.perts.iperts.inits1.ibl.close();
	msisa.maps.perts.iperts.inits1.out.close();
